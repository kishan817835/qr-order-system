...
const requireSuperAdmin = async (req, res, next) => {
  try {
    ...
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'restaurant_secret');
...
router.get('/restaurants/:restaurantId/details', requireSuperAdmin, async (req, res) => {
  try {
    ...
    const [
      ...
      Table.countDocuments({ restaurant_id: restaurantId, status: 'occupied' }),
      User.countDocuments({ 
      ...
      User.findOne({ restaurant_id: restaurantId, role: 'admin' })
        .select('name email phone last_login createdAt')
    ]);
    ...
    const restaurantsWithInfo = await Promise.all(
      restaurants.map(async (restaurant) => {
        const [tablesCount, staffCount, ordersCount, admin] = await Promise.all([
          Table.countDocuments({ restaurant_id: restaurant._id, is_active: true }),
          User.countDocuments({ 
            restaurant_id: restaurant._id, 
            role: { $ne: 'customer' },
            is_active: true 
          ...
          User.findOne({ restaurant_id: restaurant._id, role: 'admin' })
            .select('name email')
        ]);
