...
router.get('/items/restaurant/:restaurantId', async (req, res) => {
  try {
    ...
    const items = await MenuItem.find({ 
      restaurant_id: restaurantId, 
      is_active: true 
    }).populate('category_id').sort({ category_id: 1, sort_order: 1, name: 1 });
...
router.post('/items', [
  ...
  try {
    ...
    const populatedItem = await MenuItem.findById(item._id).populate('category_id');
...
router.patch('/items/:itemId/availability', [
  body('availability').isIn(['available', 'out_of_stock', 'discontinued']).withMessage('Invalid availability status')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
