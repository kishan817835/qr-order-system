...
router.post(
  ...
  [
    ...
    body("total_amount")
      .isFloat({ min: 0 })
      .withMessage("Valid total amount required"),
  ],
  async (req, res) => {
    try {
      ...
      for (const orderItem of items) {
        ...
        estimatedTime = Math.max(
          estimatedTime,
          menuItem.preparation_time + orderItem.quantity * 2,
        );
      }
    ...
    try {
      ...
      if (status === "completed" || status === "cancelled") {
        ...
        if (status === "completed") {
          ...
          order.actual_preparation_time = Math.round(
            (Date.now() - prepStartTime) / (1000 * 60),
          );
        }
      }
