...
export default function DeliveryDashboard() {
  ...
  const handlePickupOrder = (orderId: string) => {
    ...
    const hasActiveDelivery = orders.some(
      (order) => order.status === "picked-up" || order.status === "on-the-way",
    );

    if (hasActiveDelivery) {
      alert(
        "You already have an active delivery! Please complete current delivery first.",
      );
      return;
    }
  ...
  const notifyStakeholders = (orderId: string) => {
    ...
    console.log(
      "📱 SMS to Customer:",
      `Your order ${orderId} has been picked up by ${deliveryProfile.name} (${deliveryProfile.phone}). Track: ${window.location.origin}/track/TRK${Date.now()}`,
    );
    console.log(
      "🔔 Admin Notification:",
      `Order ${orderId} picked up by ${deliveryProfile.name}`,
    );
    console.log(
